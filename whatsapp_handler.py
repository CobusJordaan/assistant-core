...
async def handle_whatsapp_message(
    ...
    if not client_id:
        # Structured website lead? The contact-form "Send via WhatsApp"
        # button already collected the customer's name/email/phone/area, so
        # we skip both the language menu and the new-connection prompt
        # and log the sales-lead ticket immediately.
        if _is_structured_website_lead(body):
            session_store.mark_greeted(from_number)
            return await _handle_website_lead(
                session_store, session, from_number, body, lang,
            )
...
# Marker prefix used by the website's "Send via WhatsApp" button. The
# contact form composes a structured message starting with this string;
# when the bot sees it on a cold inbound it knows the customer has
# already supplied name + email + phone + location + area + message via
# the form, so we skip the language menu and the new-connection prompts
# and log the sales-lead ticket immediately.
_WEBSITE_LEAD_MARKER = "Draadloze website enquiry"


def _is_structured_website_lead(body: str) -> bool:
    """True when the inbound message was generated by the website's
    'Send via WhatsApp' button. The marker phrase appears near the start
    so we don't have to scan very far."""
    if not body:
        return False
    return _WEBSITE_LEAD_MARKER in body[:300]


def _extract_lead_field(body: str, label: str) -> str:
    """Best-effort `Label: value` extractor for the structured message
    body — used to fish the customer's name out for the confirmation."""
    needle = f"{label}:"
    idx = body.lower().find(needle.lower())
    if idx == -1:
        return ""
    line_end = body.find("\n", idx)
    line = body[idx + len(needle):line_end if line_end != -1 else len(body)]
    return line.strip()
...
async def _handle_website_lead(
    session_store: WhatsAppSessionStore,
    session,
    from_number: str,
    body: str,
    ...
    """Cold-entry path for website-form-driven enquiries.

    The customer used the contact form's 'Send via WhatsApp' button, so the
    inbound message already carries all the lead fields (name, email,
    phone, area, etc.). We log a sales-lead ticket immediately and reply
    with the ticket number — no language menu, no follow-up prompts.
    """
    # Lock language to whatever was detected on the body so subsequent
    # replies stay consistent. The structured message contains the
    # customer's free-text portion which is the strongest language signal.
    if lang in ("en", "af") and not session.language:
        session_store.lock_language(from_number, lang)
        session.language = lang
        session.language_locked = True

    customer_name = _extract_lead_field(body, "Name") or "there"
    ...
    try:
        ticket = billing_client.create_support_ticket(
            client_id=None,
            category="general",
            subject="New connection enquiry (website)",
            message=body,
            source="whatsapp",
            source_phone=from_number,
        )
        if ticket.get("success"):
            ticket_number = ticket.get("ticket_number", "")
            reply = t(lang, "website_lead_logged",
                      ticket=ticket_number, name=first_name)
            logger.info("Website lead %s logged for %s (name=%r)",
                        ticket_number, from_number, customer_name)
            session_store.update_after_reply(from_number, body, reply)
            return reply
        logger.warning("Website lead ticket failed for %s: %s",
                       from_number, ticket.get("error"))
    except Exception as e:
        logger.error("Website lead ticket error for %s: %s",
                     from_number, e, exc_info=True)
